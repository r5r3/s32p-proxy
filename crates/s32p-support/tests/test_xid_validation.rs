...
use http::Uri;
use s32p_support::classifier::{QueryParams, classify_with_headers};
...
fn test_xid_validation_logic() {
    ...
    let query =
        QueryParams::from_uri(&Uri::from_str("/bucket/key?x-id=GetObject&other=param").unwrap());
...
fn test_xid_with_multipart_operations() {
    ...
    assert!(matches!(
        class.op,
        s32p_support::classifier::S3Op::Multipart(
...
fn test_xid_with_versioning_operations() {
    ...
    assert!(matches!(
        class.op,
        s32p_support::classifier::S3Op::Versioning(
...
fn test_xid_with_bucket_admin_operations() {
    ...
    assert!(matches!(
        class.op,
        s32p_support::classifier::S3Op::BucketAdmin(
...
fn test_non_effective_attributes() {
    ...
    let uri =
        Uri::from_str("/bucket/key?content-type=text/plain&cache-control=max-age=3600").unwrap();
    let class = classify_with_headers("GET", &uri, None, &[]);
    assert!(matches!(
        class.op,
        s32p_support::classifier::S3Op::Read(s32p_support::classifier::ReadOp::GetObject)
    ...
    let uri = Uri::from_str(
        "/bucket/key?content-type=text/plain&cache-control=no-cache&content-disposition=inline",
    )
    .unwrap();
    let class = classify_with_headers("GET", &uri, None, &[]);
    assert!(matches!(
