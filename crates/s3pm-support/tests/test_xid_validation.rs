use s3pm_support::classifier::{classify_with_headers, QueryParams};
use http::Uri;
use std::str::FromStr;
...
fn test_xid_with_different_operations() {
    ...
    assert!(matches!(class.op, s3pm_support::classifier::S3Op::Write(
        s3pm_support::classifier::WriteOp::DeleteObject
    )));
    ...
    let class = classify_with_headers("GET", &uri, None, &[]);
    assert!(matches!(class.op, s3pm_support::classifier::S3Op::Read(
        s3pm_support::classifier::ReadOp::ListBuckets
    )));
}
...
fn test_xid_with_multipart_operations() {
    ...
    assert!(matches!(class.op, s3pm_support::classifier::S3Op::Multipart(
...
fn test_xid_with_versioning_operations() {
    ...
    let class = classify_with_headers("GET", &uri, None, &[]);
    assert!(matches!(class.op, s3pm_support::classifier::S3Op::Versioning(
        s3pm_support::classifier::VersioningOp::GetBucketVersioning
    )));
}
...
fn test_xid_with_bucket_admin_operations() {
    ...
    let class = classify_with_headers("PUT", &uri, None, &[]);
    assert!(matches!(class.op, s3pm_support::classifier::S3Op::BucketAdmin(
        s3pm_support::classifier::BucketAdminOp::CreateBucket
    )));
    ...
    let class = classify_with_headers("DELETE", &uri, None, &[]);
    assert!(matches!(class.op, s3pm_support::classifier::S3Op::BucketAdmin(
        s3pm_support::classifier::BucketAdminOp::DeleteBucket
    )));
}
