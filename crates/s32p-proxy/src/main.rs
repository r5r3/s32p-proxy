...
struct S3ProxyApp {
    ...
    /// `request_filter` invocation. `Mutex<Option<_>>` lets us hand the
    /// listener over exactly once when the loop starts.
...
impl ProxyHttp for S3ProxyApp {
    ...
    async fn request_filter(&self, session: &mut Session, ctx: &mut Self::CTX) -> PResult<bool> {
        ...
        // 2.0) SSE reject. The gateway has no encryption path — any server-
        // side encryption variant (SSE-C, SSE-S3, SSE-KMS, SSE-KMS-DSSE)
        // would result in plaintext storage while the client believes its
        // body was encrypted at rest. Reject at the proxy with an AWS-shaped
        // 400 InvalidRequest before any routing decision, so the failure
        // surfaces immediately regardless of which class/action the request
        // would otherwise hit. No SigV4 pre-check needed — this is a
        // structural header-shape gate that exposes no routing or user state.
        if let Some(kind) = s32p_support::utils::detect_unsupported_sse(&req.headers) {
            tracing::info!(
                method = req.method.as_str(),
                path = req.uri.path(),
                access_key = %user.access_key,
                sse_kind = ?kind,
                "rejected server-side-encryption request (not supported by this proxy)"
            );
            let resp = s32p_support::s3resp::sse_not_supported(kind, Some(req.uri.path()));
