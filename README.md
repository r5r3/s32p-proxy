...
- **No object-level metadata storage** (with one scoped exception). `s32p-gateway` does not persist `x-amz-meta-*` headers or a client-supplied `Content-Type`; PUT silently drops them and HEAD/GET return only what can be derived from the file (size, inode-as-ETag, mtime). The POSIX interop story (a POSIX user creates a file under `bucket.data_path` and an S3 client reads it intact) requires that the *absence* of any auxiliary state still yield a valid object. Object tagging is permitted because it satisfies that constraint: tags live in a single `user.s32p.tags` xattr, an absent xattr means "no tags", and a freshly POSIX-created file is a tagless-but-valid S3 object. `x-amz-meta-*` does not get the same treatment because its larger, less-bounded shape doesn't fit cleanly in a single xattr and offers no comparable client need (tags solve the pipeline-state-marker use case; `x-amz-meta-*` does not). Same principle as `PutObjectAcl` being a no-op when the requested ACL matches current POSIX state.
